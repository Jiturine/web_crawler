...
def get_movie_info(id):
    ...
    try:
        ...
        # 导演
        try:
            movie_director = movie_infos.find('span', {"class": "pl"}, string='导演').parent.get_text(
                strip=True).replace('导演:', '').strip()
        except (AttributeError, TypeError):
            movie_director = "未知导演"

        # 编剧
        try:
            movie_scriptwriter = movie_infos.find('span', {"class": "pl"}, string='编剧').parent.get_text(
                strip=True).replace('编剧:', '').strip()
        except (AttributeError, TypeError):
            movie_scriptwriter = "未知编剧"

        # 主演
        try:
            movie_star = movie_infos.find('span', {"class": "pl"}, string='主演').parent.get_text(
                strip=True).replace('主演:', '').strip()
        except (AttributeError, TypeError):
            movie_star = "未知主演"
        ...
        # 上映日期
        try:
            movie_date = movie_infos.find(
                'span', {"property": "v:initialReleaseDate"}).get_text().strip()
        except (AttributeError, TypeError):
            movie_date = "未知上映日期"

        # IMDb
        try:
            movie_IMDb = movie_infos.find(
                'span', {"class": "pl"}, string='IMDb:').next_sibling.get_text().strip()
        except (AttributeError, TypeError):
            movie_IMDb = "未知IMDb"

        # 评分
        try:
            movie_rating = bs.find("div", {"id": "interest_sectl"}).find(
...
def get_movie_comments(id, count):
    ...
    try:
        ...
        while (i < count):
            urls.append(
                base_url + "comments?start={0}&limit=20&status=P&sort=new_score".format(i))
            i += 20
        comments = []
        comment_count = 0
        for url in urls:
            try:
                ...
                for comment_item in comment_items:
                    ...
                    try:
                        comment_time_str = comment_item.find(
                            'span', {"class": "comment-time"}).get_text(strip=True)
                    ...
                    try:
                        comment_content = comment_item.find(
                    ...
                    try:
                        star_class = comment_item.find(
                            'span', {"class": lambda x: x and x.startswith('allstar')})['class'][0]
                        comment_rating = int(star_class[-2])
                    except (AttributeError, KeyError, ValueError):
                        try:
                            match = re.search(r'allstar(\d{2})', str(
                                comment_item.find("span", {"class": "comment-info"})))
                            comment_rating = int(
                    ...
                    try:
                        comment_isuseful = int(comment_item.find(
                            "span", {"class": "votes vote-count"}).get_text())
                    except (AttributeError, ValueError):
                        comment_isuseful = 0
