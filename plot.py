...
def plot_book_comment_histogram(book_data_list):
    ...
    for book_data in book_data_list:
        ...
        comment_texts = [comment["comment_content"]
                         for comment in comment_list]
        classify_result = classify(comment_texts)
        positive_num = len(
            [comment for comment in classify_result if comment["is_positive"] == 1])
        negative_num = len(
            [comment for comment in classify_result if comment["is_positive"] == 0])
    ...
    save_path = os.path.join(
        "static", f"book_comment_histogram_{book_data['book_id']}.png")
...
def plot_book_comment_wordcloud(book_data):
    ...
    plt.axis("off")
    save_path = os.path.join(
        "static", f"book_comment_wordcloud_{book_data['book_id']}.png")
    plt.savefig(save_path)
    return save_path
...
def plot_movie_comment_histogram(movie_data_list):
    ...
    plt.legend()
    save_path = os.path.join(
        "static", f"movie_comment_histogram_{movie_data['movie_id']}.png")
    plt.savefig(save_path)
    return save_path
...
def plot_movie_comment_wordcloud(movie_data):
    ...
    plt.axis("off")
    save_path = os.path.join(
        "static", f"movie_comment_wordcloud_{movie_data['movie_id']}.png")
    plt.savefig(save_path)
    return save_path
